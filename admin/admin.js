...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const adminErrorMessage = document.getElementById("admin-error-message"); // Elementos do formulário de produto
  ...
  const productListAdmin = document.getElementById("product-list-admin"); // Função para mostrar/esconder painel
  ...
  function togglePanel(isLoggedIn) {
    if (isLoggedIn) {
      ...
      adminPanel.classList.add("hidden"); // Limpa o formulário ao deslogar
  ...
  btnAdminLogin.addEventListener("click", async () => {
    ...
    try {
      ...
      const user = userCredential.user; // Verifica se é admin no Firestore

      const adminDoc = await db.collection("admins").doc(user.email).get();
      if (adminDoc.exists) {
        console.log("Login de administrador bem-sucedido"); // onAuthStateChanged irá cuidar de mostrar o painel
      } else {
        await auth.signOut();
        adminErrorMessage.innerText =
  ...
  productForm.fileInput.addEventListener("change", () => {
    ...
    if (file) {
      productForm.fileNameDisplay.innerText = file.name; // Limpa o campo de URL manual, pois priorizamos o arquivo
      productForm.productImageUrlManual.value = ""; // Pré-visualização do novo arquivo
      ...
      productForm.fileNameDisplay.innerText = "Nenhum arquivo selecionado."; // Se estiver em modo edição, mantém a imagem existente (se houver)
  ...
  productForm.btnSave.addEventListener("click", async () => {
    ...
    const file = productForm.fileInput.files[0];
    const manualUrlInput = productForm.productImageUrlManual.value.trim(); // VALIDAÇÃO BÁSICA
    ...
    productForm.btnSave.innerText = id ? "Atualizando..." : "Salvando..."; // Variável para armazenar a URL final da imagem.

    let imageUrl = "";
    try {
      if (file) {
        // 1. Prioridade: Upload de novo arquivo para o Cloudinary
        productForm.btnSave.innerText = "Enviando Imagem..."; // Prepara os dados para o Cloudinary
        const formData = new FormData();
        formData.append("file", file);
        formData.append("upload_preset", UPLOAD_PRESET); // Faz o upload usando Fetch API
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Falha no Cloudinary. Erro: ${errorData.error.message || response.statusText}. Verifique seu CLOUD_NAME e UPLOAD_PRESET (deve ser Unsigned).`,
          );
        }
      ...
      const data = {
        ...
        description: productForm.description.value.trim(), // Garante que o preço seja um número, ou 0 se for inválido
      ...
      await showAlert(
        "Erro ao salvar produto. Verifique as Regras de Segurança (Firestore/Storage) e se você é um Admin. Erro: " +
          error.message,
      );
  ...
  async function editProduct(id) {
    try {
      ...
      const product = doc.data(); // 1. Limpa o estado do arquivo e display ao iniciar a edição
      productForm.fileInput.value = "";
      productForm.fileNameDisplay.textContent = "Manter imagem atual"; // 2. Preenche os campos do formulário
  ...
  async function deleteProduct(id) {
    ...
    if (confirmed) {
      try {
        ...
        await showAlert(
          "Erro ao excluir produto. Verifique as Regras de Segurança do Firestore. Erro: " +
            error.message,
        );
